...
def load_raw_data(data_path):
    ...
    features_all_list = []
    elems_boundary_list = []
    features_boundary_list = []
    nodes_boundary_list = []

    elem_dim = 2
    for i in range(ndata):
        nodes_all_list.append(np.load(path_all + "/nodes_%05d" % (i) + ".npy"))
        elems = np.load(path_all + "/elems_%05d" % (i) + ".npy")
        elems_all_list.append(np.concatenate(
            (np.full((elems.shape[0], 1), elem_dim, dtype=int), elems), axis=1))
        features_all_list.append(np.load(path_all + "/features_%05d" % (i) + ".npy"))

    elem_dim = 1
    for i in range(ndata):
        nodes_boundary_list.append(np.load(path_boundary + "/nodes_%05d" % (i) + ".npy"))
        elems = np.load(path_boundary + "/elems_%05d" % (i) + ".npy")
        elems_boundary_list.append(np.concatenate(
            (np.full((elems.shape[0], 1), elem_dim, dtype=int), elems), axis=1))
        features_boundary_list.append(np.load(path_boundary + "/features_%05d" % (i) + ".npy"))

    return nodes_all_list, elems_all_list, features_all_list, nodes_boundary_list, elems_boundary_list, features_boundary_list
...
if PREPROCESS_DATA:
    ...
    ndata_list = [5, 5, 5, 5]
    radius_list = [0.05, 0.05, 0.05, 0.05, 0.05]
    should_mix = False

    for ndata, shape, radius in zip(ndata_list, shapes_list, radius_list):
    ...
    if should_mix:
        print(f"\nMixing data", flush=True)
        data_dict = mix_data(folder + '/preprocessed_data/', ndata_list, shapes_list)
        save_path = folder + '/preprocessed_data/bno_mixed_data.npz'
        np.savez(save_path, **data_dict)

    exit()
else:
    print(f"{'%' * 40} Testing ExtBNO on the Poisson Dataset {'%' * 40}", flush=True)
...
aux_train, aux_test = (mask_x_train, nodes_x_train, nodes_y_train, weights_y_train), (mask_x_test, nodes_x_test, nodes_y_test, weights_y_test)
