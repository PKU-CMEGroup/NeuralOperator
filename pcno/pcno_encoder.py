...
def PCNO_train(x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, save_model_name="./PCNO_model", checkpoint_path=None):
    ...
    for ep in range(current_epoch, epochs):
        ...
        for x, y, node_mask, nodes, node_weights, directed_edges, edge_gradient_weights in train_loader:
            ...
            if normalization_y:
                out_decode = y_normalizer.decode(out)
                y_decode = y_normalizer.decode(y)
            out_decode = out_decode * node_mask
            loss1 = myloss(out_decode.view(batch_size_,-1), y_decode.view(batch_size_,-1))


            loss.backward()
            optimizer.step()
            train_rel_l2_encoder += loss.item()
            train_rel_l2 += loss1.item()

        test_l2 = 0
        test_rel_l2 = 0
        test_rel_l2_encoder = 0
        ...
        with torch.no_grad():
            for x, y, node_mask, nodes, node_weights, directed_edges, edge_gradient_weights in test_loader:
                ...
                out = out * node_mask
                test_rel_l2_encoder += myloss(out.view(batch_size_,-1), y.view(batch_size_,-1)).item()
        ...
        train_rel_l2_encoder/= n_train
        test_l2 /= n_test
        test_rel_l2/= n_test
        test_rel_l2_encoder/= n_test
        ...
        t2 = default_timer()
        print("Epoch : ", ep, " Time: ", round(t2-t1,3)," Rel. Train L2 Loss (Encoder) : ", train_rel_l2_encoder," Rel. Train L2 Loss : ", train_rel_l2,
               " Rel. Test L2 Loss (Encoder) : ", test_rel_l2_encoder, " Rel. Test L2 Loss : ", test_rel_l2, " Test L2 Loss : ", test_l2,
