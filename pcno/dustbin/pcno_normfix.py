...
class SpectralConv(nn.Module):
    ...
    def forward(self, x, bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0):
        ...
        return x
class SpectralConv_normfix(nn.Module):
    def __init__(self, in_channels, out_channels, modes):
        super(SpectralConv_normfix, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        nmodes, ndims, nmeasures = modes.shape
        self.modes = modes
    ...
    def forward(self, x, bases_c, bases_s, bases_0, wbases_c_list, wbases_s_list, wbases_0_list):
        '''
        Compute Fourier neural layer
            Parameters:  
                x                   : float[batch_size, in_channels, nnodes]
        ...
        for i,(wbases_c, wbases_s, wbases_0) in enumerate(zip(wbases_c_list, wbases_s_list, wbases_0_list)):
            if i ==0 and i< len(wbases_0_list) -1:
                x_c_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_c)
                x_s_hat = -torch.einsum("bix,bxkw->bikw", x, wbases_s)
                x_0_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_0)       
                x_current = torch.einsum("bokw,bxkw->box", x_0_hat, bases_0)  + 2*torch.einsum("bokw,bxkw->box", x_c_hat, bases_c) -  2*torch.einsum("bokw,bxkw->box", x_s_hat, bases_s)      
                coffe = torch.norm(x)/torch.norm(x_current)
                x_current = x_current * coffe
            elif len(wbases_0_list) == 1:
                x_c_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_c)
                x_s_hat = -torch.einsum("bix,bxkw->bikw", x, wbases_s)
                x_0_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_0)                  
            else:
                x_c_hat +=  torch.einsum("bix,bxkw->bikw", x - x_current, wbases_c)
                x_s_hat -= torch.einsum("bix,bxkw->bikw", x - x_current, wbases_s)
                x_0_hat +=  torch.einsum("bix,bxkw->bikw", x - x_current, wbases_0)
                if i < len(wbases_0_list) -1:
                    x_current = torch.einsum("bokw,bxkw->box", x_0_hat, bases_0)  + 2*torch.einsum("bokw,bxkw->box", x_c_hat, bases_c) -  2*torch.einsum("bokw,bxkw->box", x_s_hat, bases_s)      
                    coffe = torch.norm(x)/torch.norm(x_current)
                    x_current = x_current * coffe   
...
class PCNO_NORMFIX(nn.Module):
    def __init__(
        ...
        self.sp_convs = nn.ModuleList(
            [
                SpectralConv_normfix(in_size, out_size, modes)
    ...
    def forward(self, x, aux, nx = None):
        """
        Forward evaluation. 
        1. Lift the input to the desire channel dimension by self.fc0 .
        2. len(layers)-1 layers of the point cloud neural layers u' = (W + K + D)(u).
        ...
        if nx != None:
            wbases_c_list, wbases_s_list, wbases_0_list = normfix_multi(wbases_c, wbases_s, wbases_0, nx, self.modes * self.sp_L, self.sigma_list, self.k_list)
        ...
        for i, (speconv, w, gw) in enumerate(zip(self.sp_convs, self.ws, self.gws)):
            x1 = speconv(x, bases_c, bases_s, bases_0, wbases_c_list, wbases_s_list, wbases_0_list)
            x2 = w(x)
