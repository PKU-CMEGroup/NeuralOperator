...
from pcno.pcno_spL import PCNO, compute_gradient, compute_Fourier_bases, SpectralConv, scaled_sigmoid, CombinedOptimizer, Combinedscheduler_OneCycleLR
...
class TPCNO(PCNO):
    def __init__(
        ...
        super().__init__(ndims, modes, nmeasures, layers, fc_dim, in_dim, out_dim, inv_L_scale_hyper, act)
        if 1:
            self.filterw = SpectralConv(self.layers[-1], self.layers[-1], modes)
        """
    ...
    def forward(self, x, t, aux):
        ...
        x = self.filterw(x,bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0)
        x = x.permute(0, 2, 1)
