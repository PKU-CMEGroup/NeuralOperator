#!/usr/bin/env python3
"""Render the registered A31 binary position/phase rollout bundles."""
...
from scripts.time_dependent_no.evaluate_pcno_binary_position_phase_rollout import (
    ANIMATION_CONTRACT,
    ANIMATION_SCHEMA,
    RESULT_SCHEMA,
    WORKING_ID,
...
RENDER_SCHEMA = "pcno_sp19_binary_position_phase_animation_render_v1"
...
def _load_contract(
    ...
    if (
        ...
        raise ValueError("rollout is not the registered unrendered A31 execution")
    if (
        ...
        or rollout.get("animation_bundle_manifest_sha256")
        != sha256_file(manifest_path)
    ):
        raise ValueError("animation manifest differs from the registered A31 contract")
    return rollout, manifest


def render(
    rollout_path: Path, manifest_path: Path, output_dir: Path
) -> dict[str, Any]:
    ...
    payload = with_payload_sha256(
        {
            ...
            "interpretation": (
                "Positive signed relative improvement means the A31 candidate has "
