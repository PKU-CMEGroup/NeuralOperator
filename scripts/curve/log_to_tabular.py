...
def extract_test_losses(log_file_path):
    ...
    try:
        ...
        default_scaled = float(f"{default_value * 100:.4g}")
        two_circles_scaled = float(f"{two_circles_value * 100:.4g}")
...
def TABLE4_generate():
    # 表格4:固定kernel_types="exterior_laplacian_neumann", layers="64,64,64,64,64,64",act=gelu
    n_train_values = [1000,2000,4000,8000]
    k_max_values = [8,16,32,64]
    kernel = "exterior_laplace_neumann"
    layers = "64,64,64,64,64,64"
    act = "gelu"
    string0 = "\\begin{table}[htbp]\n\t\\begin{center}\n\t\t\\begin{tabular}{|c|c|c|c|c|}\n\t\t\t\\hline\n\t\t\t\\diagbox{$p$}{$n$}  & $1000$ & $2000$ & $4000$ & $8000$\\\\ \\hline\n"
    string_list = [string0]
    for k_max in k_max_values:
        row_string = f"\t\t\t{k_max}"
        for n_train in n_train_values:
        ...
        string_list.append(row_string)
    string_list.append("\t\t\\end{tabular}\n\t\\end{center}\n")
    string_list.append(f"\t\\caption{{5 layer exterior_laplacian_neumann:  Relative $L_2$ error ($\\times 10^{{-2}}$) between the reference and predicted results, with single circle and two circles.}}\n")
    string_list.append("\\end{table}\n")
    return ''.join(string_list)
