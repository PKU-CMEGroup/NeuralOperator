...
def normgradient_bases(wbases_c, wbases_s, wbases_0, nx, modes):
    temp = torch.einsum("bxd,kdw->bxkw", nx, modes/torch.norm(modes, dim = 0, keepdim = True))
...
class PCNO_NORMFIX(nn.Module):
    ...
    def forward(self, x, aux, nx):
        ...
        wbases_c_g, wbases_s_g, wbases_0_g = normgradient_bases(wbases_c, wbases_s, wbases_0, nx, self.modes * self.sp_L)

        
        x = self.fc0(x)
        x = x.permute(0, 2, 1)

        for i, (speconv, w, gw1, gw2, speconv_g) in enumerate(zip(self.sp_convs, self.ws, self.gw1s, self.gw2s, self.sp_convs_g)):
            ...
            x4 = speconv_g(x, bases_c, bases_s, bases_0, wbases_c_g, wbases_s_g, wbases_0_g)
            x = x1 + x2 + x3 + x4
