...
def mul_hier_lr_M0(
    ...
    # (blocks, 2, rank, rank), (batch, blocks, 2, parts, rank)
    # -> (batch, blocks, 2, parts, rank)
    x = torch.einsum("ndrs,bndpr->bndps", M, x)
...
class DiagLrGreen(nn.Module):
    def __init__(self, modes, in_channels, out_channels, rank, head):
        ...
        self.scale_U1 = 1 / (in_channels * rank * head)
        self.scale_U2 = 1 / (out_channels * rank * head)

        self.U1 = nn.Parameter(
            self.scale_U1
            * torch.rand(in_channels, rank, modes, head, dtype=torch.float)
        )
        self.U2 = nn.Parameter(
            self.scale_U2
            * torch.rand(out_channels, rank, modes, head, dtype=torch.float)
        )
...
class DiagLrMGreen(nn.Module):
    def __init__(self, modes, in_channels, out_channels, rank, head):
        ...
        self.M = nn.Parameter(
            self.scale_M * torch.rand(rank, rank, modes, head, dtype=torch.float)
        )
        self.U2 = nn.Parameter(
            self.scale_U2
            * torch.rand(out_channels, rank, modes, head, dtype=torch.float)
...
class HierLrGreen(nn.Module):
    def __init__(self, modes, in_channels, out_channels, rank, level):
        ...
        self.scale_U1 = 1 / (in_channels * rank)
        self.scale_U2 = 1 / (out_channels * rank)
    ...
    def forward(self, x):
        ...
        x_add = torch.zeros(batch_size, self.out_channels, self.modes, device=x.device)
        for l in range(1, self.level):
            num_blocks = self.modes // 2**l
            size_part = 2 ** (l - 1)
            x_blocks = x.reshape(batch_size, self.in_channels, num_blocks, 2, size_part)
...
class HierLrM0Green(nn.Module):
    def __init__(self, modes, in_channels, out_channels, rank, level):
        super(HierLrM0Green, self).__init__()
        ...
        for l in range(1, level):
            ...
            self.M.append(
                nn.Parameter(
                    (
                        self.scale_M
                        * torch.rand(num_blocks, 2, rank, rank, dtype=torch.float)
                    )
                )
            )
            self.U2.append(
...
class LrGkNO(nn.Module):
    ...
    def _choose_green(self, idx):
        ...
        elif green_type == "HierLrM0Green":
            modes = self.modes_gk[idx]
            level = self.levels[idx]
            rank = self.ranks_h[idx]
            return HierLrM0Green(modes, in_channels, out_channels, rank, level)
