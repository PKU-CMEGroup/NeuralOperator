...
def PCNO_train(x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, save_model_name="./PCNO_model", checkpoint_path=None, res_dims = []):
    ...
    print(f'Use residual dims = {res_dims}')
    ...
    for ep in range(current_epoch, epochs):
        ...
        for x, y, node_mask, nodes, node_weights, directed_edges, edge_gradient_weights in train_loader:
            ...
            res_target = x[..., res_dims]
            batch_size_ = x.shape[0]
            optimizer.zero_grad()
            out = model(x, (node_mask, nodes, node_weights, directed_edges, edge_gradient_weights)) #.reshape(batch_size_,  -1)
            if normalization_y:
                out[...,0:1] = y_normalizer.decode(out[...,0:1])
                y = y_normalizer.decode(y)
            out[...,0:1] = out[...,0:1] * node_mask #mask the padded value with 0,(1 for node, 0 for padding)
            loss = myloss(out[...,0:1].view(batch_size_,-1), y.view(batch_size_,-1))
            loss_res = myloss(out[...,1:].reshape(batch_size_,-1), res_target.reshape(batch_size_,-1))
            loss_all = loss + loss_res
            
            loss_all.backward()

            optimizer.step()
            train_rel_l2 += loss.item()
            res_rel_l2 += loss_res.item()
        ...
        with torch.no_grad():
            for x, y, node_mask, nodes, node_weights, directed_edges, edge_gradient_weights in test_loader:
                ...
                if normalization_y:
                    ...
                    y = y_normalizer.decode(y)
                out[...,0:1] = out[...,0:1]*node_mask #mask the padded value with 0,(1 for node, 0 for padding)
                test_rel_l2 += myloss(out[...,0:1].view(batch_size_,-1), y.view(batch_size_,-1)).item()
                test_l2 += myloss.abs(out[...,0:1].view(batch_size_,-1), y.view(batch_size_,-1)).item()
        ...
        t2 = default_timer()
        print("Epoch : ", ep, " Time: ", round(t2-t1,3), " Rel. Train L2 Loss : ", train_rel_l2, " Rel. Test L2 Loss : ", test_rel_l2, " Rel. Res L2 Loss : ", res_rel_l2,
