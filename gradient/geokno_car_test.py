...
from gradient.geo_utility import preprocess_data
...
def load_data(data_path="../data/car"):
    ...
    npy_normal_files = sorted(
        glob.glob(os.path.join(dataset_folder_normal, "*.npy")))
    nodes, max_nnodes = [], 0
    elems, elem_dim = [], 2
    features = []
...
if CONVERT_DATA:
    print("Loading data")
    data_path = "../data/car"
    max_nnodes, nodes_list, elems_list, features_list = load_data(
        data_path=data_path)
    ...
    np.savez("../data/car/geokno_triangle_equal_weight_data.npz", nnodes=nodes, node_mask=node_mask, nodes=nodes,
    ...
    np.savez("../data/car/geokno_triangle_data.npz", nnodes=nodes, node_mask=node_mask, nodes=nodes, node_weights=node_weights,
             features=features, directed_edges=directed_edges, edge_gradient_weights=edge_gradient_weights)
    exit()
else:
