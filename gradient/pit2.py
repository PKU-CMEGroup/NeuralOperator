...
from gradient.pit1 import CholeskyLinear
...
class PhyGraphConv(nn.Module):
    ...
    def forward(self, f, nodes, node_weights, edges_index, edge_gradient_weights):
        ...
        df = compute_gradient(f, edges_index, edge_gradient_weights).permute(
            0, 2, 1).reshape(batch_size, max_nnodes, self.in_channels, self.ndims)
        f = self.mlp(f)
        f = f.permute(0, 2, 1)
        ...
        df = torch.einsum("bncd,bn->bncd", df, sum)

        f_src = f[torch.arange(batch_size).unsqueeze(1), src]
        dx = nodes[torch.arange(batch_size).unsqueeze(
            1), src] - nodes[torch.arange(batch_size).unsqueeze(1), target]
        f_src_x = torch.einsum("bec,bed->becd", f_src, dx)

        message = torch.einsum("becd,be->becd", f_src_x, edges_feature)

        out = torch.zeros(
            (batch_size, max_nnodes, self.out_channels, self.ndims), dtype=f.dtype, device=f.device)
        out = torch.einsum("bncd,bncd->bnc", out.scatter_add_(dim=1, src=message,
                           index=target.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, self.out_channels, self.ndims).to(torch.int64)), df)
