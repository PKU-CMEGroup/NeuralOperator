...
#     tpc = ax[0].triplot(nodes[:, 0], nodes[:, 1], elems[:, 1:])
#     ax[0].scatter(nodes[:, 0], nodes[:, 1], color="red", s=0.1)
#     ax[0].scatter(nodes[:, 0], nodes[:, 1], color="black", s=0.1)
#     x1, x2, y1, y2 = -0.5, 1.5, -0.5, 1.5  # subregion of the original image
#     axins = ax[0].inset_axes(
#         [0.3, 0.6, 0.6, 0.6],
#         xlim=(x1, x2), ylim=(y1, y2), xticklabels=[], yticklabels=[])
#     axins.triplot(nodes[:, 0], nodes[:, 1], elems[:, 1:])
...
#         ax[1].plot(x_coords, y_coords, linewidth=0.1)
#     plt.show()
