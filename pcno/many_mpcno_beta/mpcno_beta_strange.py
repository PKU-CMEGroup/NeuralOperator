...
class SpectralConv(nn.Module):
    def __init__(self, in_channels, out_channels, modes, beta_dim=0):
        ...
        # Linear map input dim: in_channels (from Fourier transform) + beta_dim
        lin_in = in_channels + beta_dim
        self.scale = 1 / (lin_in * out_channels)

        self.weights_c = nn.Parameter(
            self.scale
            * torch.rand(
                lin_in, out_channels, nmodes, nmeasures, dtype=torch.float
            )
        )
        ...
        self.weights_0 = nn.Parameter(
            self.scale
            * torch.rand(
                lin_in, out_channels, 1, nmeasures, dtype=torch.float
            )
        )


    def forward(self, x, bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0, beta=None):
        '''
        Compute Fourier neural layer
            Parameters:  
                ...
                beta (optional)     : float[batch_size, beta_dim]
                    When provided, beta is appended in the frequency domain before
                    the linear map, i.e. the pipeline is:
                        FFT(x) -> cat([FFT(x), beta]) -> linear -> iFFT

            Return :
                x                   : float[batch_size, out_channels, nnodes]
        '''    
        # Fourier transform (project onto bases)
        x_c_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_c)   # [B, in_ch, nmodes, nmeasures]
        x_s_hat = -torch.einsum("bix,bxkw->bikw", x, wbases_s)
        x_0_hat =  torch.einsum("bix,bxkw->bikw", x, wbases_0)    # [B, in_ch, 1, nmeasures]

        # Append beta in the frequency domain (broadcast over modes/measures)
        if beta is not None and self.beta_dim > 0:
            # beta: [B, beta_dim]  ->  [B, beta_dim, nmodes, nmeasures]
            nmodes_cur = x_c_hat.shape[2]
            nmeasures_cur = x_c_hat.shape[3]
            beta_freq   = beta[:, :, None, None].expand(-1, -1, nmodes_cur, nmeasures_cur)
            beta_freq_0 = beta[:, :, None, None].expand(-1, -1, 1, nmeasures_cur)

            x_c_hat = torch.cat([x_c_hat, beta_freq],   dim=1)   # [B, in_ch+beta_dim, nmodes, nmeasures]
            x_s_hat = torch.cat([x_s_hat, beta_freq],   dim=1)
            x_0_hat = torch.cat([x_0_hat, beta_freq_0], dim=1)   # [B, in_ch+beta_dim, 1, nmeasures]

        weights_c, weights_s, weights_0 = self.weights_c, self.weights_s, self.weights_0
        
        # Linear map in frequency domain
...
class MPCNO_Beta(nn.Module):
    def __init__(
        ...
        self.sp_convs = nn.ModuleList(
            [
                SpectralConv(in_size, out_size, modes, beta_dim=beta_dim)
                for in_size, out_size in zip(
                    self.layers, self.layers[1:]
                )
            ]
    ...
    def forward(self, x, beta, aux):
        ...
        3. Project from the channel space to the output space by self.fc1 and self.fc2 .
            ...
            Parameters: 
                x : Tensor float[batch_size, max_nnomdes, in_dim] 
                    Input data
                beta: Tensor float[batch_size, max_nnomdes, beta_dim]
                    Also Input data
                aux : list of Tensor, containing
                    node_mask : Tensor int[batch_size, max_nnomdes, 1]  
                                1: node; otherwise 0
        ...
        for i, (speconv, spw, spconvnw, spconvadjnw, w, grad_layer, geo_emb) in enumerate(zip(self.sp_convs, self.sp_ws, self.sp_convs_nws, self.sp_convs_adj_nws, self.ws, self.grad_layers, self.geo_embs)):
            ...
            if self.layer_selection['geointegral']:
                x1 = speconv( spconvnw(  torch.cat([x] + [x * outward_normals[:, i:i+1, :] for i in range(outward_normals.size(1))], dim=1)  ), bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0, beta=beta)
                x1 = spw(x1) + spconvadjnw(torch.cat([x1 * outward_normals[:, i:i+1, :] for i in range(outward_normals.size(1))], dim=1))
            else:
                x1 = speconv(x, bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0, beta=beta)
                x1 = spw(x1)
