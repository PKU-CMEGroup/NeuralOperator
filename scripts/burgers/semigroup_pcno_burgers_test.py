...
#x_train1, x_test1 = torch.cat((features[:n_train, :, [0,1]], nodes_input[:n_train, ...], node_rhos[:n_train, ...]), -1), torch.cat((features[-n_test:, :, [0,1]], nodes_input[-n_test:, ...], node_rhos[-n_test:, ...]),-1)
#x_train2, x_test2 = torch.cat((features[:n_train, :, [2,3]], nodes_input[:n_train, ...], node_rhos[:n_train, ...]), -1), torch.cat((features[-n_test:, :, [2,3]], nodes_input[-n_test:, ...], node_rhos[-n_test:, ...]),-1)
#aux_train       = (node_mask[0:n_train,...], nodes[0:n_train,...], node_weights[0:n_train,...], directed_edges[0:n_train,...], edge_gradient_weights[0:n_train,...])
#aux_test        = (node_mask[-n_test:,...],  nodes[-n_test:,...],  node_weights[-n_test:,...],  directed_edges[-n_test:,...],  edge_gradient_weights[-n_test:,...])

#y_train1, y_test1 = features[:n_train, :, [2]],     features[-n_test:, :, [2]]
#y_train2, y_test2 = features[:n_train, :, [4]],     features[-n_test:, :, [4]]

#x_train, x_test, y_train, y_test, aux_train, aux_test = torch.cat((x_train1,x_train2),0), torch.cat((x_test1,x_test2),0), torch.cat((y_train1,y_train2),0), torch.cat((y_test1,y_test2),0), torch.cat((aux_train,aux_train),0), torch.cat((aux_test,aux_test),0)
