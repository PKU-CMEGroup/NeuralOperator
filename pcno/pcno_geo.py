...
class PCNO(nn.Module):
    def __init__(
        ...
        self.geows = nn.ModuleList(
            [
                nn.Conv1d((ndims+1)*len(geo_dims), out_size, 1)
    ...
    def forward(self, x, aux):
        ...
        geo_grad = compute_gradient(geo, directed_edges, edge_gradient_weights)  # float[batch_size, geo_dims*ndims, nnodes]
        geo = torch.cat([geo, geo_grad], dim=1)  # float[batch_size, geo_dims + geo_dims*ndims, nnodes]

        x = self.fc0(x)
        x = x.permute(0, 2, 1)

        for i, (speconv, w, gw, geow) in enumerate(zip(self.sp_convs, self.ws, self.gws, self.geows)):
