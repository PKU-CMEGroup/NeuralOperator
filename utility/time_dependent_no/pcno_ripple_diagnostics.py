...
LOCAL_BASIS_CENTER_COUNT = 289
LOCAL_BASIS_RADIUS_FACTOR = 2.0
...
def deterministic_farthest_point_centers(
    nodes: np.ndarray,
    *,
    center_count: int,
) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    """Select geometry-only centers after independently normalizing each span.

    The first center is the lowest-index node nearest (0.5, 0.5). Each
    subsequent center maximizes its minimum Euclidean distance to the selected
    set, with numpy.argmax providing a deterministic lowest-index tie break.
    No field, target, node type, or diagnostic weight enters this construction.
    """

    positions = np.asarray(nodes, dtype=np.float64)
    if positions.ndim != 2 or positions.shape[1] != 2:
        raise ValueError("local basis nodes must have shape [N,2]")
    if not np.isfinite(positions).all():
        raise ValueError("local basis nodes must be finite")
    if center_count < 1 or center_count > positions.shape[0]:
        raise ValueError("center_count must lie in [1,N]")
    minimum = positions.min(axis=0)
    span = np.ptp(positions, axis=0)
    if np.any(span <= 0.0):
        raise ValueError("local basis requires a positive coordinate span")
    normalized = (positions - minimum) / span
    if np.unique(normalized, axis=0).shape[0] < center_count:
        raise ValueError("local basis requires at least center_count unique nodes")

    midpoint = np.full(2, 0.5, dtype=np.float64)
    first = int(np.argmin(np.sum((normalized - midpoint) ** 2, axis=1)))
    center_indices = np.empty(center_count, dtype=np.int64)
    ...
    for index in range(1, center_count):
        ...
        next_center = int(np.argmax(candidates))
        if candidates[next_center] <= 0.0:
            raise ValueError("farthest-point sampling exhausted unique nodes")
        center_indices[index] = next_center
        selected[next_center] = True
        squared_distance = np.sum(
            (normalized - normalized[next_center]) ** 2,
            axis=1,
        )
    ...
    fill_distance = float(np.sqrt(np.max(minimum_squared_distance)))
    if not np.isfinite(fill_distance) or fill_distance <= 0.0:
        raise ValueError("local basis fill distance must be finite and positive")
    return {
        "center_count": int(center_count),
        "first_center_rule": "nearest_span_normalized_(0.5,0.5)_lowest_index_tie",
        "first_center_index": first,
        "fill_distance_h": fill_distance,
        "coordinate_minimum": minimum.tolist(),
        "coordinate_span": span.tolist(),
        "geometry_only": True,
    }, {
        "normalized_positions": normalized,
        "center_indices": center_indices,
        "normalized_centers": normalized[center_indices],
        "minimum_center_distance": np.sqrt(minimum_squared_distance),
    }


def wendland_c2_partition_of_unity_basis(
    normalized_positions: np.ndarray,
    normalized_centers: np.ndarray,
    *,
    fill_distance: float,
    radius_factor: float = LOCAL_BASIS_RADIUS_FACTOR,
) -> tuple[dict[str, Any], np.ndarray]:
    """Build a compact Wendland-C2 partition of unity with radius R=2h."""

    positions = np.asarray(normalized_positions, dtype=np.float64)
    centers = np.asarray(normalized_centers, dtype=np.float64)
    if positions.ndim != 2 or positions.shape[1] != 2:
        raise ValueError("normalized positions must have shape [N,2]")
    if centers.ndim != 2 or centers.shape[1] != 2 or centers.shape[0] < 1:
        raise ValueError("normalized centers must have shape [K,2]")
    if not np.isfinite(positions).all() or not np.isfinite(centers).all():
        raise ValueError("normalized positions and centers must be finite")
    if not np.isfinite(fill_distance) or fill_distance <= 0.0:
        raise ValueError("fill_distance must be finite and positive")
    if not np.isfinite(radius_factor) or radius_factor <= 1.0:
        raise ValueError("radius_factor must exceed one to guarantee coverage")
    radius = float(radius_factor * fill_distance)
    distance = np.linalg.norm(
        positions[:, None, :] - centers[None, :, :],
        axis=-1,
    )
    scaled_distance = distance / radius
    positive_part = np.maximum(1.0 - scaled_distance, 0.0)
    raw_basis = positive_part**4 * (4.0 * scaled_distance + 1.0)
    row_sum = raw_basis.sum(axis=1)
    covered = np.isfinite(row_sum) & (row_sum > 0.0)
    if not np.all(covered):
        raise ValueError("Wendland-C2 supports do not cover every node")
    basis = raw_basis / row_sum[:, None]
    support_fraction = np.mean(raw_basis > 0.0, axis=0)
    partition_error = float(np.max(np.abs(basis.sum(axis=1) - 1.0)))
    summary = {
        "kernel": "wendland_c2_(1-r)^4_+(4r+1)",
        "partition": "row_normalized_partition_of_unity",
        "radius_rule": "R=2h",
        "radius_factor": float(radius_factor),
        "radius": radius,
        "fill_distance_h": float(fill_distance),
        "finite": bool(np.isfinite(basis).all()),
        "covered_node_count": int(np.count_nonzero(covered)),
        "node_count": int(positions.shape[0]),
        "partition_of_unity_max_row_error": partition_error,
        "column_support_fraction_median": float(np.median(support_fraction)),
        "column_support_fraction_p90": float(np.quantile(support_fraction, 0.9)),
        "column_support_fraction_max": float(np.max(support_fraction)),
    }
    return summary, basis


def weighted_basis_projection_audit(
    basis: np.ndarray,
    weights: np.ndarray,
    field: np.ndarray,
    *,
    ...
    """Project a field through weighted normal equations for any real basis."""

    functions = np.asarray(basis, dtype=np.float64)
    values = np.asarray(field, dtype=np.float64)
    if values.ndim == 1:
        values = values[:, None]
    if functions.ndim != 2 or not np.isfinite(functions).all():
        raise ValueError("basis must be a finite [N,K] array")
    if values.ndim != 2 or not np.isfinite(values).all():
        raise ValueError("field must be a finite [N,C] array")
    if functions.shape[0] != values.shape[0]:
        raise ValueError("basis and field must share the node axis")
    if not 0.0 < rcond < 1.0:
        raise ValueError("rcond must lie in (0,1)")
    mass = normalized_node_weights(weights, name="weighted basis projection")
    if mass.shape[0] != functions.shape[0]:
        raise ValueError("basis and weights must share the node axis")
    gram = functions.T @ (mass[:, None] * functions)
    right_hand_side = functions.T @ (mass[:, None] * values)
    coefficients = np.linalg.pinv(gram, rcond=rcond) @ right_hand_side
    reconstruction = functions @ coefficients

    diagonal = np.diag(gram)
    if np.any(diagonal <= 0.0):
        raise ValueError("weighted basis Gram matrix has a nonpositive diagonal")
    inverse_sqrt = 1.0 / np.sqrt(diagonal)
    correlation = gram * inverse_sqrt[:, None] * inverse_sqrt[None, :]
    correlation = 0.5 * (correlation + correlation.T)
    eigenvalues = np.linalg.eigvalsh(correlation)
    ...
    target_max = np.max(values, axis=0)
    target_range = np.maximum(target_max - target_min, 1e-12)
    result: dict[str, Any] = _weighted_error_summary(
        reconstruction,
        values,
        mass,
    )
    result.update(
        {
            "basis_columns": int(functions.shape[1]),
            "numerical_rank": rank,
            "rank_fraction": float(rank / functions.shape[1]),
            "correlation_condition_number": condition,
            "rcond": float(rcond),
            "overshoot_fraction_by_component": (
                np.maximum(np.max(reconstruction, axis=0) - target_max, 0.0)
                / target_range
            ).tolist(),
            "undershoot_fraction_by_component": (
                np.maximum(target_min - np.min(reconstruction, axis=0), 0.0)
                / target_range
            ).tolist(),
        }
    )
...
def localized_wendland_projection_audit(
    nodes: np.ndarray,
    weights: np.ndarray,
    field: np.ndarray,
    *,
    regions: Mapping[str, np.ndarray] | None = None,
    center_count: int = LOCAL_BASIS_CENTER_COUNT,
    rcond: float = 1e-10,
) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    """Run the fixed geometry-only FPS/Wendland-C2 projection diagnostic."""
    ...
    return {
        ...
        "claim_boundary": (
            "fixed no-training projection diagnostic; it is not a learned "
            "architecture or rollout result"
        ),
    }, {
        **center_arrays,
        "basis": basis,
