...
def generate_2Dnormal_data(ndata,npoints,freqx,freqf):
    ...
    for i in tqdm(range(ndata)):
        ...
        fout[i,:,:] = integral_operator(points[i,:,:], elems, nebla_log_normal_kernel, fin[i,:,:], points[i,:,:], num_quad_points=3).reshape((npoints,1))
...
np.savez('data/2D_neblalogr_data_3_3_10000.npz', nodes_list = points, elems_list = elems, features_list = features, normal_list = normal)