...
from pcno.mpcno_beta_lowrank import compute_Fourier_modes, MPCNO_Beta, MPCNO_train_multidist_beta
...
parser.add_argument('--hyper_hidden',      type=int,   default=64,
                    help='Hidden width of Tucker hyper-MLPs for SpectralConv (default 64)')
parser.add_argument('--rank',              type=int,   default=8,
                    help='Tucker rank r for SpectralConv weight offsets (default 8)')
...
parser.add_argument('--beta_dim',          type=int,   default=1,
                    help='Dimension of beta input (default 1, scalar beta)')
...
n_tucker_params = sum(p.numel()
                      for sc in model.sp_convs
                      for p in list(sc.hyper_net.parameters())
                             + [sc.A_c, sc.A_s, sc.A_0,
                                sc.B_c, sc.B_s, sc.B_0,
                                sc.V_c, sc.V_s, sc.V_0])
n_film_params = 0
...
print(f'  Tucker hyper (SpectralConv): {n_tucker_params:,}  ({100*n_tucker_params/n_params:.2f}%)')
