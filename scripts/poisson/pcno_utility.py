...
def load_raw_data(data_path, ndata):
    ...
    for i in range(ndata//2):
        ...
        # features have laplace solution, boundary node indicator, distance function, boundary condition
...
def plot_solution(problem_type, data_path_pref, shape, i, sol_pred, ax, fig):
    ...
    lc = ax[-3].tripcolor(nodes[:, 0], nodes[:, 1], elems, features[:, f_idx])
    plot_colorbar(fig, lc, ax[-3])
    
    lc = ax[-2].tripcolor(nodes[:, 0], nodes[:, 1], elems, sol_pred)
    plot_colorbar(fig, lc, ax[-2])
