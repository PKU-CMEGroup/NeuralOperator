...
def generate_curves_data_panel_beta(
        n_data, N,
        beta_range=(0.5, 1.0),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--f_mode", type=str, default="2d", choices=["1d", "2d"])
    parser.add_argument("--f_k", type=int, default=6, help="Used only when --f_mode=1d")
    parser.add_argument("--fourier_kmax", type=int, default=16)
    ...
    f_random_config = [args.f_mode] if args.f_mode == "2d" else [args.f_mode, args.f_k]
    fourier_domain_lengths = (args.Lx, args.Ly)
    ...
    if not args.two_curves:
        nodes_list, elems_list, features_list, betas_list = generate_curves_data_panel_beta(
            ...
            fourier_kmax=args.fourier_kmax,
            fourier_domain_lengths=fourier_domain_lengths,
        )
    else:
    ...
    default_name = (
        ...
        f"_{f_random_config[-1]}_{args.kernel_type}_k_max_{args.fourier_kmax}_beta_random"
        f"_panel_{tag}.npz"
    )
