...
def truncate(nx, x, wbases_c, wbases_s, wbases_0, bases_c, bases_s, bases_0, modes, sigma):
    ...
    x_trun = torch.einsum("bokw,bxkw->box", x_0_hat, bases_0)  + 2*torch.einsum("bokw,bxkw->box", x_c_hat, bases_c) -  2*torch.einsum("bokw,bxkw->box", x_s_hat, bases_s) 
    
    return x_trun

def truncate_multi(nx, x, wbases_c, wbases_s, wbases_0, bases_c, bases_s, bases_0, modes, sigma_list, k_max_list):
    n_dim = nx.shape[-1]
    for i,(sigma, k_max) in enumerate(zip(sigma_list, k_max_list)):
        num_modes = ((2*k_max+1)**n_dim- 1 )//2
        if i ==0 :
            x_trun = truncate(nx, x, wbases_c[:,:,:num_modes,:], wbases_s[:,:,:num_modes,:], wbases_0, bases_c[:,:,:num_modes,:], bases_s[:,:,:num_modes,:], bases_0, modes[:num_modes], sigma)
        else:
            x_trun += truncate(nx, x-x_trun, wbases_c[:,:,:num_modes,:], wbases_s[:,:,:num_modes,:], wbases_0, bases_c[:,:,:num_modes,:], bases_s[:,:,:num_modes,:], bases_0, modes[:num_modes], sigma)
    return x_trun
...
class PCNO_NORMFIX(nn.Module):
    ...
    def forward(self, x, aux, nx = None):
        ...
        for i, (speconv, w, gw) in enumerate(zip(self.sp_convs, self.ws, self.gws)):
            ...
            x2 = w(x)
            x_trun = truncate_multi(nx, x, wbases_c, wbases_s, wbases_0, bases_c, bases_s, bases_0,self.modes * self.sp_L, self.sigma_list, self.k_list)
            g = compute_gradient(x_trun, directed_edges, edge_gradient_weights)
            x3 = gw(self.softsign(g))
