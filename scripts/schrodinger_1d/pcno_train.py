...
def preprocess_data(data, n_train, n_test, preprocess_data:bool = True, pcno_data_file = "../../data/schrodinger_1d/pcno_data.npz"):
    '''
    参数：
    ...
    node_weights = node_measures / (2*np.pi) 
...
def setup_model(in_dim, out_dim, device, checkpoint_path = None):
    nlayers = 6
    ndim=1
    L_b = 2*np.pi * 2.1  #at least 2 times larger
...
if __name__ == "__main__":
    ...
    x_train, aux_train, y_train, x_test, aux_test, y_test = preprocess_data(data, n_train, n_test, preprocess_data=False, pcno_data_file = "../../data/schrodinger_1d/pcno_schrodinger1d_"+V_type+"_data.npz")
    ...
    train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = PCNO_train(x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, save_model_name="./PCNO_model_V"+V_type)
