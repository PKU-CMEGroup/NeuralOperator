...
normals = features[...,[1,2]].unsqueeze(-1)
...
loc_modes = compute_Fourier_modes(ndim, [k_max_local, k_max_local], [L_local, L_local])
loc_modes = torch.tensor(loc_modes, dtype=torch.float).to(device)

model = PCNO(ndim, modes, nmeasures=1, loc_modes = loc_modes,
             layers=[128,128],
             fc_dim=128,               #128,
...
if model.loc_modes is not None:
    print(f'Using local modes with shape: {model.loc_modes.shape}', " L_local = ", L_local, " kmax_local = ", k_max_local, flush = True)
