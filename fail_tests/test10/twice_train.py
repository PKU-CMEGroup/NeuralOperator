...
# load configs
###################################
with open('config_2D_10.yml', 'r', encoding='utf-8') as f:
...
kernel_mode = config_model['kernel_mode']
GkNN_mode = config_model['GkNN_mode']
model_ref = torch.load('model/fullH')
pca_H = model_ref.H1.reshape(kernel_mode,GkNN_mode*GkNN_mode).cpu()
pca_H = pca_H.detach().numpy()
U, S, VT = np.linalg.svd(pca_H.T)

H1 = torch.from_numpy(U[:,:kernel_mode].T).to('cuda')
H1 = H1.reshape(kernel_mode,GkNN_mode,GkNN_mode)
H2 = 0
config['train']['epochs']=500
model = HGkNN(bases_list, H1, H2, **config_model).to(device)
print('params:',count_params(model))

print("Start twice training ", "layer_type: ",config_model,config['train'], flush = True)
train_rel_l2_losses, test_rel_l2_losses = HGkNN_train(
    x_train, y_train, x_test, y_test, config, model, save_model_name='model/fullH_twice'
)