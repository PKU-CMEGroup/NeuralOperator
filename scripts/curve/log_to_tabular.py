...
def get_the_loss_from_log(kernel_type, k_max, n_train, num_grad, layers, act, prefix = "1_1_5_2d", grad="True", geo="True", geoint="True",
    ...
    tuple: (default_scaled, two_circles_scaled) 或 None（如果提取失败）
    """
    log_dir = f"log/{prefix}_{kernel_type}/{num_grad}_{layers}_{act}/"
    ...
    return extract_test_losses(log_file_path)

def TABLE1_generate(num_grad):
    ...
    for kernel in kernel_types:
        ...
        for k_max in k_max_values:
            result = get_the_loss_from_log(kernel_type=kernel, k_max=k_max, num_grad=num_grad, layers=layers, act=act, n_train=n_train)
            if result:
    ...
    string_list.append(f"\t\\caption{{num-grad={num_grad},Linear train on 8000:  Relative $L_2$ error ($\\times 10^{{-2}}$) between the reference and predicted results, with single circle and two circles.}}\n")
    string_list.append("\\end{table}\n")

    return ''.join(string_list)

def TABLE2_generate(num_grad):
    ...
    for kernel in kernel_types:
        ...
        string_list.append(row_string)
    string_list.append("\t\t\\end{tabular}\n\t\\end{center}\n")
    string_list.append(f"\t\\caption{{num-grad={num_grad},5 layer train on 8000:  Relative $L_2$ error ($\\times 10^{{-2}}$) between the reference and predicted results, with single circle and two circles.}}\n")
    string_list.append("\\end{table}\n")

    return ''.join(string_list)

def TABLE3_generate(num_grad):
    ...
    for kernel in kernel_types:
        ...
        string_list.append(row_string)
    string_list.append("\t\t\\end{tabular}\n\t\\end{center}\n")
    string_list.append(f"\t\\caption{{num-grad={num_grad},5 layer $p = 32$:  Relative $L_2$ error ($\\times 10^{{-2}}$) between the reference and predicted results, with single circle and two circles.}}\n")
    string_list.append("\\end{table}\n")
    return ''.join(string_list)


with open("table.txt", "w") as f:
    lines = TABLE1_generate(num_grad=1)
    f.write(lines)
    f.write("\n\n")
    lines = TABLE1_generate(num_grad=3)
    f.write(lines)
    f.write("\n\n")
    lines = TABLE2_generate(num_grad=1)
    f.write(lines)
    f.write("\n\n")
    lines = TABLE2_generate(num_grad=3)
    f.write(lines)
    f.write("\n\n")
    lines = TABLE3_generate(num_grad=1)
    f.write(lines)
    f.write("\n\n")
    lines = TABLE3_generate(num_grad=3)
    f.write(lines)
    f.write("\n\n")