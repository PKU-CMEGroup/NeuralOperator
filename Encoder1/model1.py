...
class GkNN(nn.Module):
    def __init__(self, bases_list, **config):
        ...
        self.encoder = DoubleAttentionEncoder(
            self.in_dim,
            self.layers_dim[0],
            self.wbases_pca_in,
            self.bases_pca_out,
...
class DoubleAttentionEncoder(nn.Module):
    def __init__(
        self, in_channels, out_channels, wbases_V, bases_Q, modes_V, modes_Q, fc_dim
    ):
        super(DoubleAttentionEncoder, self).__init__()
        ...
        self.modes_V = modes_V
        self.modes_Q = modes_Q
        self.modes_K = modes_Q

        self.wbases_V = wbases_V[:, :modes_V]
        self.bases_Q = bases_Q[:, :modes_Q]

        # find key in the space spanned by learnable hidden bases
        self.linear_K = nn.Linear(self.in_channels, self.modes_K)
        self.norm_K = nn.LayerNorm(self.modes_K, eps=1e-05)
        # self._reset_parameters_K()

        # ffn layer
        self.fc = nn.Sequential(
            nn.Linear(self.modes_V, fc_dim),
            nn.GELU(),
            nn.Linear(fc_dim, self.out_channels),
        )

    def forward(self, x):
        """
        Input Shape: (batchsize, n, in_channels)
        Output Shape : (batchsize, n, out_channels)
        """
        # try 3: normalize K, using weighted bases
        key = self.linear_K(x)
        key = self.norm_K(key)
