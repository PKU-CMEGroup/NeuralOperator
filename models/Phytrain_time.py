...
def PhyHGkNN_train_time(
    ...
    for ep in range(epochs):
        ...
        t11= default_timer()
        print('model.train()', t11-t1,flush = True)
        ...
        torch.cuda.synchronize()
        t12= default_timer()
        print('before_forward', t12-t11,flush = True)
        to_time, forward_time,loss_time,backward_time,optimizer_time =0,0,0,0,0
        for x, y in train_loader:
            ...
            loss_time+=t5-t4
            backward_time+=t6-t5
            optimizer_time+=t7-t6
        print(f'to_time {to_time}, optimizer_time {optimizer_time}, loss_time {loss_time}, backward_time {backward_time}, optimizer_time {optimizer_time}',flush=True)
        ...
        with torch.no_grad():
            if plot_hidden_layers:
                ...
                if ep%20 ==0:
                    ...
                    fig, axs = plt.subplots(1, 8, figsize=(24, 5))     
                    for i in range(8):       
                        y0 = model.wbases[0,:,i].cpu().reshape(Nx,Ny)
                        im = axs[i].imshow(y0, cmap='viridis')
                        axs[i].set_title(f'bases{i}')
