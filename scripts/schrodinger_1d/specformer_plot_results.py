...
from specformer_train import setup_model, preprocess_periodic_data
...
if __name__ == "__main__":
    ...
    model = setup_model(in_dim, out_dim, device, checkpoint_path = "Specformer_model_V"+V_type+".pth")
    ...
    for i in range(nT):
        ...
        u[:,i+1,:, 0:out_dim] =  model( u = (x_normalizer.encode(u[:, i, :, 0:in_dim]) if normalization_x else u[:, i, :, 0:in_dim]), aux=aux) 
        if normalization_y:
    ...
    fig, axs = plt.subplots( out_dim+2, figsize=(8, 8))
    for i in range(out_dim):
        axs[i].plot(x, u_ref[0,:,i], color="C0", label="initial")
        axs[i].plot(x, u_ref[1,:,i], color="C1", label="ref. step 1", linewidth=0.5)
        axs[i].plot(x, u[1, :, i], "--", color="C1", markerfacecolor="none", linewidth=0.5)
        axs[i].plot(x, u_ref[2,:,i], color="C2", label="ref. step 2", linewidth=0.5)
        axs[i].plot(x, u[2, :, i], "--", color="C2", markerfacecolor="none", linewidth=0.5)
        
        # error
        axs[-2].plot(x, u[1, :, i] - u_ref[1,:,i], color="C1", markerfacecolor="none", linewidth=0.5)
        axs[-2].plot(x, u[2, :, i] - u_ref[2,:,i], color="C2", markerfacecolor="none", linewidth=0.5)
    ...
    axs[-1].legend()
    axs[0].legend()
    plt.savefig("Specformer_"+V_type+"_error.pdf")
    ...
    plt.show()
    plt.savefig("Specformer_"+V_type+str(nT)+"steps.pdf")
