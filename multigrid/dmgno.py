...
class MgNO(nn.Module):
    def __init__(
        ...
        self.sp_layers.append(
            GalerkinConv(num_channel_f, num_channel_u, modes, bases, wbases)
        )

        for _ in range(num_layer - 1):
            self.sp_layers.append(
                GalerkinConv(num_channel_u, num_channel_u, modes, bases, wbases)
            )
    ...
    def forward(self, u):
        ...
        for layer1, layer2, w in zip(self.sp_layers, self.conv_layers, self.ws):
            u = self.act(self.p * layer1(u) + (1 - self.p) * layer2(u) + w(u))
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            for layer in model.conv_layers:
                l2_reg += sum(torch.norm(param) for param in layer.get_weight_params())
