...
def error_plot(filename):
    ...
    # 显示图形
    plt.tight_layout()
    plt.savefig("car_shapenet_loss.pdf")
...
def predict_error():
    ...
    model = PCNO(ndim, modes, nmeasures=1,
                ...
                train_sp_L="together",
                act='gelu')
    model.load_state_dict(torch.load("PCNO_car_shapenet_model.pth", weights_only=True))
    ...
    ############################################################################
    fig, ax = plt.subplots(figsize=(6,6))
    ax.hist(test_rel_l2, bins=len(test_rel_l2)//4, alpha=0.7, color='C0', edgecolor='black')
    ...
    # 显示图形
    plt.tight_layout()
    plt.savefig("car_shapenet_loss_distribution.pdf")
    ############################################################################
    ...
    for i in [largest_error_ind, median_error_ind]:
        ...
        meshio.write("car_"+str(i)+".vtk", mesh)

error_plot("PCNO_car.log")
predict_error()