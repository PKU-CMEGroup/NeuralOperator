...
class SpectralConv_normfix(nn.Module):
    ...
    def forward(self, x, bases_c_list, bases_s_list, bases_0_list, wbases_c, wbases_s, wbases_0):
        '''
        Compute Fourier neural layer
            Parameters:  
                x                   : float[batch_size, in_channels, nnodes]
        ...
        for i,(bases_c, bases_s, bases_0) in enumerate(zip(bases_c_list, bases_s_list, bases_0_list)):
            if i ==0:
                x = torch.einsum("bokw,bxkw->box", f_0_hat, bases_0)  + 2*torch.einsum("bokw,bxkw->box", f_c_hat, bases_c) -  2*torch.einsum("bokw,bxkw->box", f_s_hat, bases_s)            
            else:  
                x+= torch.einsum("bokw,bxkw->box", f_0_hat, bases_0)  + 2*torch.einsum("bokw,bxkw->box", f_c_hat, bases_c) -  2*torch.einsum("bokw,bxkw->box", f_s_hat, bases_s)            
...
class PCNO_NORMFIX(nn.Module):
    ...
    def forward(self, x, aux, nx = None):
        ...
        if nx != None:
            bases_c_list, bases_s_list, bases_0_list = normfix_multi(bases_c, bases_s, bases_0, nx, self.modes * self.sp_L, self.sigma_list, self.k_list)
        ...
        for i, (speconv, w, gw) in enumerate(zip(self.sp_convs, self.ws, self.gws)):
            x1 = speconv(x,  bases_c_list, bases_s_list, bases_0_list, wbases_c, wbases_s, wbases_0,)
            x2 = w(x)
