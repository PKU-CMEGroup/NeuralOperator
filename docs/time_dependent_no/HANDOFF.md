...
6. [CODEX_KICKSTART_PROMPTS.md](CODEX_KICKSTART_PROMPTS.md) for A0-only agent
   kickoff prompts after human review.
7. [README.md](README.md) for onboarding, provenance, and maintained-code
   navigation.
...
2026-08-11 retention receipt is ignored with the artifact tree. The dated
...
This cleanup authorization does not authorize training, dataset-scale or
checkpoint execution, remote work, downloads, sealed-population access, or new
scientific claims. The five kickoff prompts authorize read-only A0 audits only.
Synthetic M1 work and every later stage still require the review and approvals
defined in the weekly plan.
...
After the five A0 audits and any approved synthetic M1 checks, the owner must:

1. confirm the exact B1 artifact and its boundary-information training and
   inference protocols;
2. approve at most two A2 open-checkpoint evaluations after joint M1 review;
3. choose at most one first A3 training matrix: no-gradient, boundary 2x2, or
   REALM residual comparison;
4. approve any multi-gigabyte REALM download and every AutoDL launch explicitly;
   and
...
New PCNO runs currently write source-snapshot schema v5. Schemas v2 through v5
retain their own registered file inventories and meanings. V2 binds the then-
current decision and tracker as part of strict source equality; v3 through v5
separate executable/scientific sources from provenance-only documents so later
