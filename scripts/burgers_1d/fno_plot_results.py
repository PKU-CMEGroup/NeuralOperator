...
if __name__ == "__main__":
    ...
    model = setup_model(in_dim, out_dim, device, checkpoint_path = "FNO_model.pth")
    ...
    u = u.to(device)  
    for i in range(nT):
        u[:,i+1,:, 0:out_dim] =  model( x = (x_normalizer.encode(u[:, i, :, 0:in_dim]) if normalization_x else u[:, i, :, 0:in_dim]) ) 
        if normalization_y:
    ...
    plt.show()
    plt.savefig("FNO_"+str(nT)+"steps.pdf")
