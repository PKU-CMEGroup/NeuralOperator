...
config = dict(config)
config_data, config_model, config_train = config['data'], config['model'], config['train']
...
if config_model['basis_type'] == "Fast_Fourier_Transform":
    ...
    bases = [torch.from_numpy(fbases.astype(np.float32)).to(device)]
    wbases = [torch.from_numpy(wfbases.astype(np.float32)).to(device)]
...
train_rel_l2_losses, test_rel_l2_losses, test_l2_losses, cost = FNN_train(x_train, y_train, x_test, y_test, config, model,  save_model_name=False)
