...
class lowrankHGalerkinConv(nn.Module):
    def __init__(self, bases, wbases,product,D_out,D_in, A,B,config):
        super(lowrankHGalerkinConv, self).__init__()
...
class lowrankHGalerkinConv_symdouble(nn.Module):
    def __init__(self, bases1, wbases1,bases2, wbases2,product,D_out,D_in, A,B,config):
        super(lowrankHGalerkinConv_symdouble, self).__init__()

        self.layer1 = lowrankHGalerkinConv(bases1,wbases1,product,D_out,D_in, A,B,config)
        config2 = config.copy()
        config2['GkNN_mode_in'],config2['GkNN_mode_out'] = config['GkNN_mode_out'],config['GkNN_mode_in']
        self.layer2 = lowrankHGalerkinConv(bases2,wbases2,product.transpose(0,1),D_in,D_out, B,A,config2)
...
class lowrankHGkNN(nn.Module):
    def __init__(self, bases_list ,**config):

        super(lowrankHGkNN, self).__init__()
    ...
    def _choose_layer(self, index, in_channels, out_channels, layer_type):
        if layer_type == "newHGalerkinConv":
            if not self.double_bases:
                ...
                return lowrankHGalerkinConv(bases, wbases,product,D_out,D_in, A,B,self.layer)
                ...
                B = self.B1
                product = self.product1
                return lowrankHGalerkinConv_symdouble(bases1, wbases1,bases2, wbases2,product,D_out,D_in, A,B,self.layer)                
            else:
                raise ValueError("bases type Undefined.")
        else:
