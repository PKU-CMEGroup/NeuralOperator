...
class LinearBetaSpectralConv(nn.Module):
    """
    SpectralConv where weights are linear in beta:
        W(beta) = W_base + sum_d(beta[d] * W_linear[d])
    
    When beta_dim=1:
        W(beta) = W_base + beta * W_linear
    
    This is the simplest possible beta-conditioning.
    Zero-initialise W_linear -> identical to SpectralConv at init.
    
    Args:
        in_channels  : int
        out_channels : int
        modes        : float Tensor [nmodes, ndims, nmeasures]
        beta_dim     : int   dimension of beta input
    """
    def __init__(self, in_channels, out_channels, modes, beta_dim):
        super(LinearBetaSpectralConv, self).__init__()
        ...
        # ---- Base weights (beta-independent), same as SpectralConv ----------
        ...
        # ---- Linear-in-beta weights, shape [beta_dim, ic, oc, km, w] --------
        # Zero-init -> W(beta) = W_base at init, regardless of beta
        self.weights_c_linear = nn.Parameter(torch.zeros(beta_dim, ic, oc, km, w))
        self.weights_s_linear = nn.Parameter(torch.zeros(beta_dim, ic, oc, km, w))
        self.weights_0_linear = nn.Parameter(torch.zeros(beta_dim, ic, oc,  1, w))
    ...
    def forward(self, x, beta, bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0):
        ...
        """
        # ---- Effective weights: W_base + einsum(beta, W_linear) -------------
        # beta: [B, beta_dim],  weights_*_linear: [beta_dim, ic, oc, km, w]
        # delta: [B, ic, oc, km, w]
        delta_c = torch.einsum('bd,diokw->biokw', beta, self.weights_c_linear)
        delta_s = torch.einsum('bd,diokw->biokw', beta, self.weights_s_linear)
        delta_0 = torch.einsum('bd,diokw->biokw', beta, self.weights_0_linear)

        weights_c = self.weights_c.unsqueeze(0) + delta_c  # [B, ic, oc, km, w]
        weights_s = self.weights_s.unsqueeze(0) + delta_s
        weights_0 = self.weights_0.unsqueeze(0) + delta_0
...
class MPCNO_Beta(MPCNO):
    ...
    2. **FiLM modulation** on GeoEmbedding and GradientLayer (local branches):
       h' = (1 + γ(β)) · h + δ(β)
       inserted after the geo_act in each module.
    ...
    Extra constructor args (beyond MPCNO's):
        ...
        film_hidden  : int   hidden width of FiLM generators in Geo/Grad layers (default 32)
    """
    ...
    def __init__(
        ...
        for in_size, out_size in zip(layers, layers[1:]):
            hyper_out_dim = 3 * rank   # independent of in_size, out_size, nmodes!
            ...
            hyper_net = nn.Sequential(
                ...
                nn.GELU(),
                nn.Linear(hyper_hidden, hyper_out_dim),
            )
            # Zero-init the output layer → all offsets start at 0 → model == MPCNO at init
            nn.init.zeros_(hyper_net[-1].weight)
            nn.init.zeros_(hyper_net[-1].bias)

            new_sp_convs.append(
                LinearBetaSpectralConv(in_size, out_size, modes, beta_dim))
