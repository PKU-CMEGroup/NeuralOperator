...
from pcno.pcno_normgradient import compute_Fourier_modes, PCNO_NORMFIX, PCNO_NORMFIX_train
...
nx_train = features[:n_train, :, 1:]
nx_test = features[-n_test:, :, 1:]
...
# print(f'sigma_list = {model.sigma_list}, k_list = {model.k_list}')
epochs = 500
base_lr = 5e-4 #0.001
lr_ratio = 10
...
train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = PCNO_NORMFIX_train(
    x_train, nx_train,  aux_train, y_train, x_test, nx_test,  aux_test, y_test, config, model, save_model_name="./PCNO_car_shapenet_model"
)
