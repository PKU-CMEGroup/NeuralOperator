...
def compute_Fourier_bases(grid, modes, mask):
    ...
    #modes: nk, ndim
    #mask : batchsize, 1, nx
    temp  = torch.einsum("bdxy,kld->bklxy", grid, modes) 
...
class SpectralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2):
        ...
        #     self.scale
        #     * torch.rand(
        #         in_channels, out_channels, 2*self.modes1, self.modes2, dtype=torch.float
        #     )
        # )
    ...
    def forward(self, x, wbases_c, wbases_s, wbases_0, bases_c, bases_s, bases_0):
        ...
        x = torch.fft.irfftn(out_ft0, s=(x.size(-2), x.size(-1)), dim=[2, 3])

        
        # x_c_hat =  torch.einsum("bixy,bklxy->bikl", x, wbases_c)
        # x_s_hat = -torch.einsum("bixy,bklxy->bikl", x, wbases_s)
        ...
        # weights_c, weights_s = weights12.real, weights12.imag
        # #weights_c, weights_s = self.weights_c/(size1*size2), self.weights_s/(size1*size2)
        # f_c_hat = torch.einsum("bikl,iokl->bokl", x_c_hat, weights_c) - torch.einsum("bikl,iokl->bokl", x_s_hat, weights_s)
        # f_s_hat = torch.einsum("bikl,iokl->bokl", x_s_hat, weights_c) + torch.einsum("bikl,iokl->bokl", x_c_hat, weights_s)
 
        # x = 2*torch.einsum("bokl,bklxy->boxy", f_c_hat[...,1:], bases_c[:,:,1:,...]) -  2*torch.einsum("bokl,bklxy->boxy", f_s_hat[...,1:], bases_s[:,:,1:,...]) \
        #    + torch.einsum("bokl,bklxy->boxy", f_c_hat[...,0:1], bases_c[:,:,0:1,...]) -  torch.einsum("bokl,bklxy->boxy", f_s_hat[...,0:1], bases_s[:,:,0:1,...])
        
        # print("x error : ", torch.norm(x - x0)/torch.norm(x0))
...
class GeoFNO(nn.Module):
    def __init__(
        ...
        self.sp_convs = nn.ModuleList(
            [
                SpectralConv2d(in_size, out_size, modes1, modes2)
    ...
    def forward(self, x):
        ...
        # aux = x[...,-2-self.ndim:].permute(0, 3, 1, 2)    # coord, weights, mask

        # grid, weights, mask = aux[:, 0:self.ndim, :, :], aux[:, -2:-1, None, :, :], aux[:, -1:, None , :, :]

        # nw = grid.shape[-1]*grid.shape[-2]
        # bases_c, bases_s, bases_0 = compute_Fourier_bases(grid, self.modes, mask)
        # wbases_c, wbases_s, wbases_0 = bases_c*(weights*nw), bases_s*(weights*nw), bases_0*(weights*nw)
        ...
        for i, (speconv, w) in enumerate(zip(self.sp_convs, self.ws)):
            ...
            x1 = speconv(x, None, None, None, None, None, None)
...
def GeoFNO_train(x_train, y_train, x_test, y_test, config, model, save_model_name="./GeoFNO_model"):
    ...
    if normalization_x:
        x_normalizer = UnitGaussianNormalizer(x_train, aux_dim = 2)
