...
from directionGeoKNO3 import compute_Fourier_modes, directionGeoKNO3, GeoKNO3_train
...
def load_data(data_path = "../data/car"):
    ...
    for index, i in enumerate(valid_indices):
        ...
        press = np.load(npy_pressure_files[index])
...
directionmodes = modes[:100]
model = directionGeoKNO3(ndim, modes,directionmodes,
...
config = {"train" : {"base_lr": base_lr, "weight_decay": weight_decay, "epochs": epochs, "scheduler": scheduler,  "batch_size": batch_size, 
        ...
        'plot': {'plot_hidden_layers_num': 3, 'save_figure_hidden': 'figure/car_pressure/test/', 'ndim': 3, 'if_plot': True}
        
        }


train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = GeoKNO3_train(
    x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, save_model_name="./GeoKNO_car_pressure_model.pth"
)
