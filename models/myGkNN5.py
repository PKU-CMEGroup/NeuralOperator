...
class GalerkinConv0(nn.Module):
    def __init__(self, in_channels, out_channels, modes, bases, wbases, kernel_size):
        super(GalerkinConv0, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.kernel_size:
            zeros = torch.zeros(x_hat.shape[0], x_hat.shape[1], self.kernel_size-1 ).to(x.device)
            x_hat = torch.cat((x_hat,zeros), dim=-1)
            x_hat = self.feature_in_conv(x_hat)
...
class spFeedforward(nn.Module):
    def __init__(self, in_modes,hidden_modes, out_modes, bases, wbases):
        ...
        self.feedforward = nn.Sequential(
            nn.Linear(in_modes,hidden_modes),
            nn.GELU(),
            nn.Linear(hidden_modes,out_modes)
        )
    
    def forward(self, x):
        #x.shape: bsz , channels , n 
        bases, wbases = self.bases, self.wbases
        bases = bases[:,:self.out_modes]
        
        # Compute coeffcients
        x_hat = torch.einsum("bcx,xk->bck", x, wbases)
        #x_hat.shape: bsz , channels , modes 
        x_hat = x_hat[:,:,:self.in_modes]

        x_hat = self.feedforward(x_hat)
...
class myGkNN5(nn.Module):
    def __init__(self, bases_list,**config):
        super(myGkNN5, self).__init__()
        ...
        self.spff = spFeedforward(self.spff_modes[0],self.spff_modes[1],self.spff_modes[2], self.bases_pca_out, self.wbases_pca_out)
    ...
    def _choose_layer(self, index, in_channels, out_channels, layer_type):
        if layer_type == "GalerkinConv_fourier":
            ...
            return GalerkinConv0(in_channels, out_channels, num_modes, bases, wbases, kernel_size)
        elif layer_type == "GalerkinConv_pca_out":
            num_modes = self.GkNN_modes[index]
            bases = self.bases_pca_out
            wbases = self.wbases_pca_out
            kernel_size = self.kernel_size
            return GalerkinConv0(in_channels, out_channels, num_modes, bases, wbases, kernel_size)
        elif layer_type == "GalerkinConv_pca_in":
            num_modes = self.GkNN_modes[index]
            bases = self.bases_pca_in
            wbases = self.wbases_pca_in
            kernel_size = self.kernel_size
            return GalerkinConv0(in_channels, out_channels, num_modes, bases, wbases, kernel_size)
