...
class GaborSpectralConv(nn.Module):
    def __init__(self, in_channels, out_channels, modes):
        ...
        self.centor = nn.Parameter(torch.randn(ndims, dtype=torch.float))
        self.sigma = nn.Parameter(torch.randn(ndims, dtype=torch.float))



    def forward(self, x, bases_c, bases_s, bases_0, wbases_c, wbases_s, wbases_0, modes):
        '''
        Compute Fourier neural layer
            Parameters:  
                x                   : float[batch_size, in_channels, nnodes]
        ...
        gaborweight = torch.exp(
            -torch.sum(
                (modes - self.centor.reshape(1,-1,1))**2 / (self.sigma.reshape(1,-1,1)**2 + 1e-5),   # nmodes, ndims, nmeasures
                  dim = 1
                )
