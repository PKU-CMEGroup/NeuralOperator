"""Acquire the pinned PlanarDet metadata or open train/validation manifest.

The command never discovers repository contents and rejects every path
component named ``test``.  ``--scope metadata`` is the required first pass;
``--scope open`` later verifies or downloads the complete registered open set.
"""
...
SCHEMA = "w26_l4_planardet_open_acquisition_v1"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--scope", choices=("metadata", "open"), required=True)
    return parser
...
def object_url(entry: ManifestEntry, *, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    repository = quote(PLANARDET_REPOSITORY, safe="/")
    revision = quote(PLANARDET_REVISION, safe="")
...
def _open_official_url(url: str) -> BinaryIO:
    request = Request(
        url,
        headers={"User-Agent": "NeuralOperator-PlanarDet-PD0-A2/1"},
    )
...
def acquire_open_manifest(
    ...
    *,
    ...
    if scope not in {"metadata", "open"}:
        raise ValueError("scope must be metadata or open")
    selected = (
        metadata_manifest_entries(entries) if scope == "metadata" else tuple(entries)
    )
    _preflight_acquisition_root(root, selected)
    ...
    final_rows = validate_local_open_tree(root, selected)
    return {
        "schema": SCHEMA,
        "scope": scope,
        "repository": PLANARDET_REPOSITORY,
        "revision": PLANARDET_REVISION,
        ...
        "complete_open_manifest_acquired": scope == "open",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        summary = acquire_open_manifest(args.data_root, entries, scope=args.scope)
