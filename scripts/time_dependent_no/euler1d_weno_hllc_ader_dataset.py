...
def hllc_flux_from_primitive(
    ...
    denom = np.where(np.abs(denom) < 1e-14, np.where(denom >= 0.0, 1e-14, -1e-14), denom)
    SM = (
        pR - pL
        + rhoL * uL * (SL - uL)
        - rhoR * uR * (SR - uR)
    ...
    def safe_nonzero(a: np.ndarray, eps_safe: float = 1e-14) -> np.ndarray:
        ...
        return np.where(np.abs(a) < eps_safe, np.where(a >= 0.0, eps_safe, -eps_safe), a)
    ...
    E_star_L = rho_star_L * (
        EL / rhoL
        + (SM - uL) * (SM + pL / denom_energy_L)
    )
    E_star_R = rho_star_R * (
        ER / rhoR
        + (SM - uR) * (SM + pR / denom_energy_R)
    )
...
def hlle_flux_from_primitive(
    ...
    F_hlle = (SR[..., None] * FL - SL[..., None] * FR + (SL * SR)[..., None] * (UR - UL)) / denom[..., None]
    return np.where(
        (SL >= 0.0)[..., None],
        FL,
...
def weno5_reconstruct_faces(
    ...
    beta0 = (13.0 / 12.0) * (jm2 - 2.0 * jm1 + j0) ** 2 + 0.25 * (jm2 - 4.0 * jm1 + 3.0 * j0) ** 2
    beta1 = (13.0 / 12.0) * (jm1 - 2.0 * j0 + jp1) ** 2 + 0.25 * (jm1 - jp1) ** 2
    beta2 = (13.0 / 12.0) * (j0 - 2.0 * jp1 + jp2) ** 2 + 0.25 * (3.0 * j0 - 4.0 * jp1 + jp2) ** 2
    ...
    beta0r = (13.0 / 12.0) * (jp3 - 2.0 * jp2 + jp1) ** 2 + 0.25 * (jp3 - 4.0 * jp2 + 3.0 * jp1) ** 2
    beta1r = (13.0 / 12.0) * (jp2 - 2.0 * jp1 + j0) ** 2 + 0.25 * (jp2 - j0) ** 2
    beta2r = (13.0 / 12.0) * (jp1 - 2.0 * j0 + jm1) ** 2 + 0.25 * (3.0 * jp1 - 4.0 * j0 + jm1) ** 2
...
def positivity_fallback_faces(
    ...
    """
    bad_L = (~np.isfinite(VL).all(axis=-1)) | (VL[:, 0] <= rho_floor) | (VL[:, 2] <= p_floor)
    bad_R = (~np.isfinite(VR).all(axis=-1)) | (VR[:, 0] <= rho_floor) | (VR[:, 2] <= p_floor)
    if np.any(bad_L):
...
def step_weno_hllc_ader2(
    ...
    Return (U_new, success). Ghost cells are filled inside this function.
    """
    U_work = U.copy()
    ...
    VL_half, VR_half = positivity_fallback_faces(VL_half, VR_half, V, j_faces, rho_floor, p_floor)
    ...
    V_new = conservative_to_primitive(U_new[ng:ng + nx], gamma, rho_floor=-np.inf, p_floor=-np.inf)
    ok = (
...
def random_in_range(rng: np.random.Generator, value_range: Tuple[float, float]) -> float:
...
def sample_case(
    ...
    x_disc = x_left + random_in_range(rng, sample_config.discontinuity_fraction_range) * length
...
def initialize_case(case: CaseConfig, nx: int, gamma: float, ng: int) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    U[ng:ng + nx] = primitive_to_conservative(V_phys, gamma)
        ...
        while t_now < target - 1e-14:
            ...
            if not success:
                ...
                U_candidate, ok = step_first_order_hllc(
                    U, case.left_state, dx, dt_try, gamma, ng, rho_floor, p_floor
                )
                if not ok:
                    raise RuntimeError(
...
def generate_dataset(
    ...
    np.savez_compressed(
        ...
        method=np.array("finite-volume WENO5 primitive reconstruction + HLLC flux + ADER2 predictor"),
