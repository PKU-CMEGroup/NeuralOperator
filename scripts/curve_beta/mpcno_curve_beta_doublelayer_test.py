...
from pcno.mpcno_beta_doublelayer import compute_Fourier_modes, MPCNO_Beta, MPCNO_train_multidist_beta
...
parser = argparse.ArgumentParser(
    description="Train MPCNO_Beta (double-layer spectral hyper-net) on anisotropic curve-beta data."
)
...
parser.add_argument("--beta_dim", type=int, default=1, help="Dimension of beta input")
parser.add_argument("--hyper_hidden", type=int, default=64, help="Hidden width for W1 sigma(W2 beta + b)")
parser.add_argument("--hyper_act", type=str, default="gelu", help="Activation sigma in W1 sigma(W2 beta + b)")
...
def gen_data_tensors(data_indices, nodes, features, node_mask, node_weights, directed_edges, edge_gradient_weights, betas):
    nodes_input = nodes.clone()

    x = torch.cat(
        [
            features[data_indices][..., : f_in_dim + 2],
...
(nnodes, node_mask, nodes, node_weights, node_rhos, features, directed_edges, edge_gradient_weights, betas, to_divide) = load_data_to_torch(
    data_path + f"/pcno_curve_data_1_1_5_beta({args.beta_low}, {args.beta_high})_2d_{args.kernel_type}_beta_random_panel_single.npz",
    to_divide=None,
    factor=to_divide_factor,
)
...
if n_two_curves > 0:
    (nnodes2, node_mask2, nodes2, node_weights2, node_rhos2, features2, directed_edges2, edge_gradient_weights2, betas2, _) = load_data_to_torch(
...
print(f"x_train {x_train.shape}  y_train {y_train.shape}  beta_train {beta_train.shape}", flush=True)
print(f"Test sets: {{ {', '.join([f'{lbl}: {tuple(x.shape)}' for lbl, x in zip(label_list, x_test_list)])} }}", flush=True)
print(f"beta range in train: [{beta_train.min().item():.4f}, {beta_train.max().item():.4f}]")
print(f"Domain range per dim: {torch.amax(nodes, dim=[0,1]) - torch.amin(nodes, dim=[0,1])}", flush=True)
...
print(f"beta_dim = {beta_dim}, hyper_hidden = {hyper_hidden}, hyper_act = {hyper_act}")
...
n_spectral_hyper_params = sum(
    p.numel()
    for sc in model.sp_convs
    for p in list(sc.hyper_w1.parameters()) + list(sc.hyper_w2.parameters()) + [sc.weights_c, sc.weights_s, sc.weights_0]
)
print(f"Total parameters:                         {n_params:,}")
print(f"  Double-layer spectral (incl. base W):   {n_spectral_hyper_params:,}  ({100*n_spectral_hyper_params/n_params:.2f}%)")
...
train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = MPCNO_train_multidist_beta(
    x_train,
    aux_train,
    y_train,
    beta_train,
