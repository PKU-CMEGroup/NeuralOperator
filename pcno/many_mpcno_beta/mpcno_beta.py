...
class MPCNO_Beta(nn.Module):
    def __init__(
        ...
        self.sp_convs = nn.ModuleList(
            [
                SpectralConv(in_size+beta_dim, out_size, modes)
                for in_size, out_size in zip(
                    self.layers, self.layers[1:]
                )
            ]
        ...
        self.sp_convs_nws = nn.ModuleList(
            [
                nn.Conv1d((in_size+beta_dim) * (ndims + 1), in_size+beta_dim, 1, bias=False)
                for in_size, out_size in zip(self.layers[1:], self.layers[1:])
            ]
        ) if layer_selection['geointegral'] else [None]*len(layers[1:])
        ...
        self.ws = nn.ModuleList(
            [
                nn.Conv1d(in_size+beta_dim, out_size, 1)
                for in_size, out_size in zip(self.layers, self.layers[1:])
            ]
        )
        ...
        self.grad_layers = nn.ModuleList(
            [
                GradientLayer(ndims, in_size+beta_dim, out_size, geo_act=geo_act)
                for in_size, out_size in zip(self.layers, self.layers[1:])
            ]
        ) if layer_selection['grad'] else [None]*len(layers[1:])
        ...
        self.geo_embs = nn.ModuleList(
            [
                GeoEmbedding(ndims*(ndims+1), in_size+beta_dim, out_size, geo_act=geo_act)
                for in_size, out_size in zip(self.layers, self.layers[1:])
            ]
        ) if layer_selection['geo'] else [None]*len(layers[1:])
    ...
    def forward(self, x, beta, aux):
        ...
        for i, (speconv, spw, spconvnw, spconvadjnw, w, grad_layer, geo_emb) in enumerate(zip(self.sp_convs, self.sp_ws, self.sp_convs_nws, self.sp_convs_adj_nws, self.ws, self.grad_layers, self.geo_embs)):
            ...
            beta_expanded = beta.unsqueeze(-1).expand(-1, -1, x.size(-1))   # [batch_size, beta_dim, nodes]
            x_prev = x
            x = torch.cat([x, beta_expanded], dim=1)                        # [batch_size, channels+beta_dim, nodes]
            ...
            if self.act is not None and i != length - 1:
                x = x_prev + self.act(self.scale_factor*(x1 + x2 + x_grad + x_geo))
            else:
                x = self.scale_factor*(x1 + x2 + x_grad + x_geo)
