...
def compute_Fourier_bases_eff(nodes, modes, modes_weight, max_modes):
    '''
    Compute Fourier bases for the whole space
    Fourier bases are cos(kx), sin(kx), 1
        ...
        Parameters:  
            nodes        : float[batch_size, nnodes, ndims]
            modes        : float[nmodes, ndims, nmeasures]
            modes_weight: float[max_modes//2,nmodes - max_modes//2]
            max_modes: int
            
        Return :
            bases_c, bases_s : float[batch_size, nnodes, max_modes, nmeasures]
            bases_0 : float[batch_size, nnodes, 1, nmeasures]
    '''

    half_modes = max_modes // 2

    temp = torch.einsum("bxd,kdw->bxkw", nodes, modes) 
    bases_c, bases_s = torch.cos(temp), torch.sin(temp)

    bases_c = torch.cat((bases_c[:,:,:half_modes,:], torch.einsum("lk,bxkw->bxlw", modes_weight, bases_c[:,:,half_modes:,:]) ),dim=2)
    bases_s = torch.cat((bases_s[:,:,:half_modes,:], torch.einsum("lk,bxkw->bxlw", modes_weight, bases_s[:,:,half_modes:,:]) ),dim=2)
...
class SpectralConv_eff(nn.Module):
    def __init__(self, in_channels, out_channels, modes, max_modes):
        super(SpectralConv_eff, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        nmodes, ndims, nmeasures = modes.shape
        self.modes = modes
        ...
        self.weights_c = nn.Parameter(
            self.scale
            * torch.rand(
                in_channels, out_channels, max_modes, nmeasures, dtype=torch.float
            )
        )
        self.weights_s = nn.Parameter(
            self.scale
...
class PCNO_EFF(nn.Module):
    def __init__(
        ...
        self.sp_convs = nn.ModuleList(
            [
                SpectralConv_eff(in_size, out_size, modes, max_modes)
    ...
    def forward(self, x, aux):
        ...
        bases_c,  bases_s,  bases_0  = compute_Fourier_bases_eff(nodes, self.modes * self.sp_L, self.modes_weight, self.max_modes)
        ...
        for i, (speconv, w, gw1, gw2) in enumerate(zip(self.sp_convs, self.ws, self.gw1s , self.gw2s)):
