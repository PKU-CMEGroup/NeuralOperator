...
class Restriction1d(nn.Module):
    ...
    def forward(self, a, u, f, bases, wbases):
        ...
        bases1 = torch.zeros(((gridsize + 1) // 2, modes)).to("cuda")
        wbases1 = torch.zeros(((gridsize + 1) // 2, modes)).to("cuda")
...
class MultiGalerkinNN1(nn.Module):
    def __init__(
        ...
        self.solve_layers = nn.ModuleList(
            [
                GalerkinPositive(a_channels, u_channels, f_channels, modes)
                for modes in modes_list
            ]
        )
        self.positives = nn.ModuleList(
            [
                Conv1dPositive(a_channels, u_channels, f_channels)
        ...
        self.prolongations = nn.ModuleList(
            [Prolongation1d(u_channels, u_channels) for _ in range(self.num_levels - 1)]
        )
    ...
    def forward(self, a):
        ...
        for level, restriction in enumerate(self.restrictions):
            df = f - self.positives[level](a, u)
            u = self.solve_layers[level](a, u, df, bases, wbases)
            u_list.append(u)
            f = f - self.positives[level](a, u)
            a, u, f, bases, wbases = restriction(a, u, f, bases, wbases)

        u = self.solve_layers[self.num_levels - 1](a, u, f, bases, wbases)
